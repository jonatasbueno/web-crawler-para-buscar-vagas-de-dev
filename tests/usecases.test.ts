...
class FakeScraper implements SourceScraper {
  constructor(public name: string, private result: Job[] | Error) {}
...
class FakeRepo implements JobRepository {
  marked: Job[] = [];
  runs: { hour: number; status: RunStatus }[] = [];
  constructor(private sent = new Set<string>(), public ranHours: number[] = []) {}
